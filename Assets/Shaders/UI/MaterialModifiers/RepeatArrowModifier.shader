...
// 重复箭头：使用居中缩放布局叠加一层残影，避免复制贴图超出 UI 框被裁切。
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                {
                    float2 repeatAxis = float2(0.0, 1.0);
                    if (_ArrowDirection > 0.5 && _ArrowDirection < 1.5)
                        repeatAxis = float2(0.0, -1.0);
                    else if (_ArrowDirection > 1.5 && _ArrowDirection < 2.5)
                        repeatAxis = float2(1.0, 0.0);
                    else if (_ArrowDirection > 2.5)
                        repeatAxis = float2(-1.0, 0.0);

                    float repeatScale = 0.88;
                    float repeatSpacing = 0.12;
                    float2 centerShift = repeatAxis * repeatSpacing * 0.5;
                    float2 baseUv = (uv + centerShift - 0.5) / repeatScale + 0.5;
                    fixed4 result = SampleMain(baseUv, IN.color);

                    float2 copyUv = (uv - centerShift - 0.5) / repeatScale + 0.5;
                    fixed4 copy = SampleMain(copyUv, IN.color);
                    copy.rgb = lerp(copy.rgb, _AuraColor.rgb, 0.45);
                    copy.a *= 0.56;
                    color = Over(copy, result);
                }
                else if (mode < 3.5)
                {
